...
在获得潜在表征 Z 之后，我们提出的模型有两个阶段：带鉴别器的正则化阶段（D）和分类阶段。我们构造了具有两个密集层的鉴别器。其功能是区分 E 与高斯分布 N（μ，σ）（实）和潜在表示 Z（假）。高斯分布的维数与潜在表示 Z 相同。目的是通过学习对更接近真实样本的样本进行编码来愚弄鉴别器。方程 6 显示了损耗函数。

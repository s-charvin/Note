...
$\left\{x \mid\left(x-x_c\right)^{\mathrm{T}} P^{-1}\left(x-x_c\right) \leqslant 1, \quad P \in \mathcal{S}_{++}^{n} \left(即 P 对称正定\right)\right\}=$

$\left\{x_c+A u \mid\|u\| \leqslant 1\right\}, \text{A 为非奇异的方阵} \}$

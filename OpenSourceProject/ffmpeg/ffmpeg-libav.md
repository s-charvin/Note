...
`ffmpeg` 的封装(Muxer)和解封装(Demuxer)操作是通过调用 `libavformat` 库来实现的. 它可以完成文件的读写(I/O)操作, 或者对网络媒体流协议数据进行解析, 从输入读取媒体流数据或者将媒体流数据写入到输出. 其中, 媒体流数据会被分割为数据包(Packet)一段段进行处理, 具体存储结构由 `AVPacket` 结构体定义.

...
标准库定义了另外一组具有更高级性质的类型，它们尚未直接实现到计算机硬件中。C++ 标准库对类型所提供的操作和性能上的需，因此对于一般应用场合来说有足够的效率。

C++ 标准库是基于模板(template)为主要工具的泛型编程方法实现的作品。
...
- 初始化方式![](C++.assets/image-20220814200619.png)![](C++.assets/image-20220814202453.png)![](C++.assets/image-20220917225456.png)


- 类操作![](C++.assets/image-20220814203226.png)![](C++.assets/image-20220917230948.png)![](C++.assets/image-20220917231045.png)![](C++.assets/image-20220917231149.png)![](C++.assets/image-20220917231220.png)![](C++.assets/image-20220917231252.png)![](C++.assets/image-20220917231319.png)
		...
		- `substr` 操作返回一个 `string`，它是原始 `string` 的一部分或全部的拷贝。可以传递给 `substr` 一个可选的开始位置和计数值。
			- ![](C++.assets/image-20220917225715.png)
		...
		- `erase` 接受下标和元素数量的版本。
			- 删除指定下标位置的单个或多个元素。
		- `insert` 接受下标和给定 C 风格字符数组的版本。
			- 添加字符数组开始指针指向地址到其结尾的值，到给定下标值之前的位置。
		- `assign` 接受 C 风格字符数组指针和所需字符数量的版本。

...
- 默认构造函数
	...
	- 定义默认构造函数
		- 一些编译器可以通过在参数列表后面写上 `= default` 来生成构造函数。
...
- 成员函数所使用成员名称查找过程
	...
	- 
	- 对象名称查找过程
		- 首先，在名字所在的块中寻找其声明语句，并且只考虑在名字的使用之前出现的声明。
			- 声明中使用的名字，包括返回类型或者参数列表中使用的名字，都必须在使用前确保可见。
		- 如果没找到，继续查找外层作用域。
			- 如果是在成员函数内没有找到，则在类内继续查找，这时外层域内（即类内）的所有成员都可以被考虑。
			- 如果是在类内没有找到，则会在定义该类的作用域中继续查找（，并且只考虑在成员名字的使用之前出现的声明或函数定义之前）。
		- 如果最终没有找到匹配的声明，则程序报错。
	- 函数名称查找过程
		- 首先，编译成员函数的声明。
		- 直到类全部可见后才编译函数体。

...
- 成员函数的声明必须在类的内部，它的定义则既可以在类的内部也可以在类的外部。定义在类内部的函数是隐式的内联函数。
...
- 默认构造函数
	- 默认构造函数无须任何实参。
	- 当类没有显式地定义任何构造函数时，那么编译器就会为我们隐式地定义一个默认构造函数：合成的默认构造函数。
		- 如果存在类内的初始值，用它来初始化成员。
		- 否则，默认初始化该成员。
	- 一旦定义了一些其他的构造函数，那么除非再定义一个默认的构造函数，否则类将没有默认构造函数。
	- 因为内置类型在函数内未被显式初始化时，其值是未定义的，因此，含有内置类型或复合类型成员的类应该在类的内部初始化这些成员，或者定义一个自己的默认构造函数。否则，用户在创建类的对象时就可能得到未定义的值。

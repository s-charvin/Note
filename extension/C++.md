...
- 声明虚函数
	- 虚函数前面需要加上 `virtual` 关键字。
		- 基类通过在其成员函数的声明语句之前加上关键字 `virtual` 可以使得该函数执行动态绑定。
		- 派生类必须在其内部对所有重新定义的虚函数进行声明。
	- 虚函数可以通过在函数形参列表之后增加一个 `override` 关键字，显式注明改写的哪个基类成员函数。



- 当我们使用指针或引用调用虚函数时，该调用将被动态绑定。根据引用或指针所绑定的对象类型不同，该调用可能执行基类的版本，也可能执行某个派生类的版本。
- 基类可以通过在其成员函数的声明语句之前加上关键字 `virtual` 使得该函数执行动态绑定, 成为虚函数。

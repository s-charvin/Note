...
大规模视觉 Transformer（ViTs）：ViT 是一种基于 Transformer 的图像分类模型，它将图像分割成一个个小块，并通过 Transformer 处理这些小块以获取全局特征。

卷积神经网络（CNN）：CNN 是一种前馈神经网络，常用于图像和语音识别、自然语言处理等领域。

有效感受野：在卷积神经网络中，每个神经元只对上一层的一部分神经元有影响，这个范围被称为感受野。有效感受野是指神经元能够看到的与其输出相关的输入区域大小。

下游任务：指建立在模型预测结果之上的更高级别的任务，如目标检测、图像分割等。

归纳偏差：机器学习模型在训练过程中通常会根据训练数据中的规律来进行推断。如果模型在训练数据中所发现的规律并不适用于测试数据，则称之为归纳偏差。

...
对于一些自动化工具, 如编译工具 TypeScript、 ESLint、TSLint、Make、Ant、Gulp、Jake、Rake 和 MSBuild 等, 能通过命令行运行, 并自动执行检查、构建、打包、测试或部署软件系统等任务。为了方便 VSCode 执行这些任务, 而不需要额外的命令行操作, 可以通过在终端菜单栏单击 `Configure Tasks/配置任务` 选择适合的模板文件来生成 `tasks.json` , 并自定义相关任务. 当需要运行任务时, 通过在终端菜单栏单击 `run task/运行任务` , 选择对应的任务名字即可.
...
            //    "reveal": "always"
            //},
            "options": { // 修改环境中一些参数的默认值, 如 `cwd` ：修改当前工作目录、`env`: xi环境变量）或 `shell`（默认 shell）的默认值。
                "cwd": "${fileDirname}" // 
            },
            "problemMatcher": [ // 
                "$gcc"

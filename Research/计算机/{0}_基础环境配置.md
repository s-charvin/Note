...
## 服务器 V2Ray 代理环境设置(Dedipath+V2Ray+Vmess+websocket+TLS+Nginx+Website+warp-go)
...
```
为域名添加 DNS 解析([Cloudflare](https://dash.cloudflare.com/))


使用脚本一键安装 [warp](https://github.com/fscarmen/warp/blob/main/README.md#warp-go-%E8%BF%90%E8%A1%8C%E8%84%9A%E6%9C%AC)
...
# 获取 warp 默认配置信息, 得到 warp 的输入端口 2000

warp-go e


Wireguard 配置文件: /opt/warp-go/wgcf.conf
 
 Sing-box 配置文件: /opt/warp-go/singbox.json
...
配置 v2ray 网络分流, 实现指定网络通过 warp 伪装

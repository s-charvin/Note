...
现在，我们有了一个具有常规帐户权限的新用户帐户。但是，我们有时可能需要执行需要管理员权限任务, 因此为了避免注销当前用户然后再重新登陆 root 帐户, 我们可以为当前普通帐户设置超级用户权限。之后只需要在执行的任务指令前面, 添加一个额外的 `sudo` 修饰指令, 就能使普通用户运行需要有管理权限的命令。 

要将超级用户权限赋予给新用户，需要将此用户添加到 **sudo** 用户组。因为在默认情况下， Ubuntu 20.04 会允许 **sudo** 组的用户成员使用 `sudo` 修饰指令.

以 **root** 身份运行以下命令可以将新用户添加到 **sudo** 组中：
...
现在，以普通用户身份登录时，你也可以通过在命令之前加上 `sudo` 来使用超级用户权限运行任务;了。 

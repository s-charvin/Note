...
- const 的变量
	- 修饰常量: 仅在作用域或文件内有效, 并且在定义是必须初始化. 
	- 修饰参数: const 的形参可以接受 const 或非 const 类型的实参.
	- 修饰成员变量: 只在定义的对象的生命周期内有效; 不能在类外定义, 只能通过每个对象建立时的构造函数的列表初始化进行初始化.
	- 修饰成员函数: 可以禁止成员函数修改对象的成员变量; 
- const 的引用
...
- const 的指针
	- 指向常量对象或非常量的常量指针![](C++.assets/image-20220813141401.png) ![](C++.assets/image-20220813141147.png)
	...
	- 顶（底）层 const![](C++.assets/image-20220813165823.png)
		- 顶层 const（top-level const）表示其本身是个常量
		- 底层 const（low-level const）表示其所指的对象是一个常量。
			- 底层 const 进行一些操作时，如拷贝，需要拷入和拷出的对象必须具有相同的底层 const 类型，或者两个对象的数据类型必须能够转换。一般来说，非常量可以转换成常量，反之则不行。![](C++.assets/image-20220813170346.png)
...
- 变量命名规范
	...
	- 不使用 C++ 内部关键字![](C++.assets/image-20220813100822.png) ![](C++.assets/image-20220813100849.png)
- [constexpr](http://c.biancheng.net/view/7781.html) 和常量表达式![](C++.assets/image-20220813173930.png) ![](C++.assets/image-20220813174039.png)
...
![](C++.assets/image-20220924092420.png)

- `insert` 成员函数，被用来向容器中添加一个元素或一个元素范围。
	- 对于只能包含不重复的关键字的类型，因此插入一个已存在的关键字元素对容器没有任何影响。
	- `insert` 有两个版本，分别接受一对迭代器，或是一个初始化器列表，这两个版本的行为类似对应的构造函数。
	- `map` 进行 `insert` 操作时，添加的元素是 `pair` 对象（参数列表 `{{}}`，`make_pair(..)`，`pair<>()` 或 `map<>::value_type(..)` 方法）。
	- insert 返回的值依赖于容器类型和参数。
		- 对于不可重复容器，如果插入的不是已有重复关键字，则返回一个 `pair` 类型对象，其中 first 成员是一个迭代器，指向具有给定关键字的元素；second 成员是一个 `bool` 值，插入成功 `bool` 部分为 `true`，如果关键字已在容器中，`bool` 部分为 `false`。
		- 对允许重复关键字的容器，调用 `insert` 总会插入一个元素，并返回一个指向新元素的迭代器。
...
![](C++.assets/image-20220924151310.png) ![](C++.assets/image-20220924151043.png) ![](C++.assets/image-20220924151054.png)
...
- `map` 和 `unordered_map` 容器提供了下标运算符和一个对应的 `at` 函数，而 `set` 、`multimap` `或unordered_multimap` 类型不支持下标。
	...
	- 下标操作可能会改变容器，因此如果只是向查找关键词，建议用搜索类成员函数。
- `find` 成员函数可以搜索某个关键词是否存在，返回找到的指向第一个关键词的迭代器。
- `count` 成员函数可以在搜索关键词的同时，统计关键词出现次数，返回关键词的出现次数。
	- 出现次数和指向第一个关键词的迭代器两者可以共同使用，找到所有关键词的位置。
- `lower_bound` 成员函数接受一个关键字，返回一个指向第一个给定关键字元素位置的迭代器。
- `upper_bound` 成员函数接受一个关键字，返回一个指向最后一个给定关键字元素之后位置的迭代器。
	- 元素不存在，则指向一个不影响排序的关键字插入位置。
- `equal_range` 成员函数接受一个关键字，返回一个迭代器 `pair` ，其中含有两个迭代器，`first` 指向第一个与关键字匹配的元素位置，`second` 指向最后一个匹配元素之后的位置。

###### 无序容器操作

![](C++.assets/image-20220924223543.png)
...
- 当解引用一个关联容器迭代器时，我们会得到一个类型为容器的 `value_type` 的值的引用。对 `map` 而言，`value_type` 是一个 pair 类型，其 first 成员保存 const 的关键字，second 成员保存值。
	- 可以改变 `pair` 的值，但不能改变关键字 `first` 成员的值。同理，`set` 中的关键字也不能修改。
...
关于无序关联容器 —— `unordered_map`、`unordered_multimap`、`unordered_set` 以及 `unordered_multiset`，使用一个哈希函数和关键字类型的 `==` 运算符进行存储和查找。

- 无序容器提供了与有序容器相同的操作，因此一般可以用其替换对应的有序容器，仅会有数据顺序的变化。
- 无序容器在存储上组织为一组桶，无序容器使用一个哈希函数将元素映射到桶，而每个桶保存零个或多个元素。
	- 因此为了访问一个元素，容器首先计算元素的哈希值，它指出应该搜索哪个桶。
	- 可以使用一个标准库中的内置类型 `hash` 模板： `hash<key_type>`，生成一些类型元素（内置类型、string，智能指针）的哈希值。
	- 可以通过重载哈希值计算函数和 `==` 比较函数，自定义可以保存在无序容器中的元素。
...
如果基类定义了一个静态成员，则在整个继承体系中只存在该成员的唯一实例，且静态成员遵循通用的访问控制规则（`public` 、`protected` 或者 `private` ）。
...
- 类派生列表（类名后紧跟的以逗号分隔的基类列表）明确指出它是从哪些基类继承而来。
	- `class Sub_class:public Base_class1,protected Base_class2,...{...}`
	- 基类列表中的基类可以使用 `public` 、`protected` 或者 `private` 关键字，控制派生类从基类继承而来的成员是否对派生类的用户可见。

- 声明虚函数
	- 虚函数必须要在其成员函数的声明语句之前加上关键字 `virtual` 关键字。
		- 基类通过 `virtual` 可以使得该函数执行动态绑定。
		- 派生类必须在其内部对所有需要重新定义的虚函数通过 `virtual` 使得该函数执行动态绑定。
		- 如果基类把一个函数声明成虚函数，则该函数在派生类中隐式地也是虚函数。
		- 关键字 `virtual` 只能出现在类内部的声明语句之前而不能用于类外部的函数定义。
		- 任何构造函数之外的非静态函数都可以是虚函数。
	- 派生类的虚函数也可以通过在函数形参列表之后、或者在 `const` 成员函数的 `const` 关键字后面、或者在引用成员函数的引用限定符后面增加一个 `override` 关键字显式注明。
...
- 构造函数
	...
	- 派生类对象最好不要在初始化之外，为基类成员赋值。

- 多层继承
	- 出现在派生列表中的是当前派生类的直接基类，而间接基类由派生类通过其直接基类继承而来。
	- 最终的派生类将包含其直接基类的子对象以及每个间接基类的子对象。
	- 在类名后跟一个关键字 `final`，如 `class Sub_class final : ...,{...};`，可以防止其他类继承。

### 派生类声明和应用

派生类的声明与其他类差别不大，声明中包含类名但是不包含它的派生列表 `class Sub_class;`。
...
通过使用动态绑定，函数可以仅通过声明基类对象类型参数，处理派生类对象。

## c/c++ 内存管理

### 操作系统内存管理

当 CPU 需要从内存中读取数据时, 数据会被加载到寄存器中, 然后通过总线传输到 CPU 中进行计算和处理. 同样, 当 CPU 需要将处理结果写入内存时, 数据会先被存储到寄存器中, 然后再通过总线传输到内存中. 

寄存器在 CPU 中扮演着临时存储器的角色, 通过它们, CPU 可以快速访问和处理数据.
- 与内存相比, 寄存器的访问速度非常快, 因为寄存器位于 CPU 内部, 可以直接访问, 这使得寄存器非常适合用于临时存储在计算过程中频繁使用的数据.
- 使用寄存器可以减少 CPU 与内存之间的数据传输次数, 从而提高计算效率, 因为从内存中读取或写入数据需要花费更多时间和资源.
- 寄存器通常具有固定的命名和地址, 这使得它们更容易在程序设计和编程时进行管理和控制. 程序员可以使用寄存器来存储和处理特定的数据, 从而更容易理解和维护代码.
- 现代 CPU 通常具有多个寄存器, 这使得程序能够同时处理多个数据, 从而提高程序的并行性, 提高计算机系统的整体性能.

CPU 通过总线向内存发送请求, 例如读取或写入数据, 内存在接收到请求后会执行相应的操作, 并通过总线向 CPU 返回所请求的数据. 为了更好地管理和控制计算机系统中的数据传输和处理, 总线通常被分为三种类型: 数据总线, 地址总线和控制总线.
- 数据总线: 用于在 CPU 和内存之间传输数据, 它决定了每次传输的数据量和数据的宽度, 一般来说, 数据总线的宽度越宽, 每次传输的数据量就越大, CPU 与内存之间的数据传输速度也就越快. 与数据总线相关的寄存器有数据寄存器（Data Register）和数据缓存寄存器（Data Buffer Register）, 它们被用于临时存储传输的数据.
- 地址总线: 用于传输内存地址信息, 使得内存能够定位到要访问的数据所在的内存地址. 地址总线的宽度决定了 CPU 可以寻址的最大内存容量. 与地址总线相关的寄存器有地址寄存器（Address Register）, 它用于存储 CPU 需要访问的内存地址.
- 控制总线：用于传输控制信号, 例如时钟信号, 读写信号, 中断信号等. 控制总线的作用是控制 CPU 和内存之间的数据传输和处理, 确保数据能够按照正确的顺序和时间传输和处理. 与控制总线相关的寄存器有控制寄存器（Control Register）, 它用于存储 CPU 的控制信息和状态信息, 如指令计数器（Instruction Counter）, 状态寄存器（Status Register）等.

CPU 和内存之间的数据交互是一个复杂的过程, 包括多个步骤和信号的传输. 
1. CPU 通过地址总线将内存地址传输给内存, 首先, CPU 中的地址寄存器将要访问的内存地址存储起来, 然后通过地址总线将地址传输给内存. 
2. 内存接收到CPU传输的地址后, 定位到对应的存储单元, 将数据存储到数据缓存寄存器中. 
3. CPU 通过控制总线向内存发出读取数据的请求, 内存响应 CPU 的请求, 将数据通过数据总线传输给 CPU. 
4. CPU 将接收到的数据存储到数据寄存器中. 
5. CPU 对数据进行处理, 将处理结果存储到数据寄存器中. 
6. CPU 通过地址总线将存储处理结果的内存地址传输给内存. 
7. 内存接收到 CPU 传输的地址后, 定位到对应的存储单元, 将处理结果存储到数据缓存寄存器中. 
8. CPU 通过控制总线向内存发出写入数据的请求, 内存响应 CPU 的请求, 将数据缓存寄存器中的数据通过数据总线写入内存中. 
9. CPU 完成了一次对内存的读写操作. 

在早期 16 位 CPU 中, 只有 16 位的寄存器, 只能用来暂存 16 位的数据/地址/指令/状态/计数, 而 16 位的地址寄存器最大只能寻址 64KB 地址范围内的内存空间, (64KB = 64 * 1024B =  $2^{16}$ ).

后面随着内存空间容量的增加, 单一的 16 位地址寄存器逐渐无法满足需求, 为了解决大内存空间的寻址问题, 出现了内存分段方式. 即将整个大内存空间划分为若干个逻辑段, 通过引入段基址寄存器和段内偏移寄存器, 使用 16 位的段基地址和段内偏移地址来表示实际的物理地址, 这样就能访问更大的内存空间了! 并且最初的段基址寄存器被分为了四种, 其中存放的段基地址分别直接指向代码段(cs), 数据段(ds), 栈段(ss)和扩展段(es). 

以 8086 为例, 其寄存器均是 16 位的, 也就是说数据总线为 16 位, 一次性能并行传输或暂存的数据只能有 16 位, 但是其地址总线却有二十位, 即最大可以寻址的内存空间的地址范围为 1024KB=1MB . 通过内存分段方式, 可以将 1MB 的内存空间分割为 1MB/64KB=16 个 64KB 大小的内存段空间(或者说 64KB 个 16B 大小的内存块). 
基地址 * 16 + 偏移地址，也就是 ABCD * 16 + 0010.


![](C++.assets/image-20230310135451.png)



但是, 在 16 位 CPU 中, 内存地址通过 16 位的段寄存器和 16 位的偏移量来表示. 这种方式的问题在于, 偏移量的大小是固定的, 因此每个段的大小也是固定的. 这导致了内存的碎片化, 同时也容易受到缓冲区溢出等安全漏洞的攻击. 

因此 32位 CPU 又引入了段描述符和段选择子, 使得每个逻辑段的大小和起始地址可以自由设置. 也就是说, 在段寄存器中存放不不再是原来真实的物理内存地址, 而是段选择子. 段选择子结构简单, 存储了当前请求的特权级别, 段描述符类型和段描述符所在索引. 处理器将索引值乘以 8 , 再加上 GDT 或者 LDT 的基地址, 就是要加载的段描述符的地址. CPU 使用段选择子来选择需要访问的逻辑段, 并将其转换为对应的段描述符, 从而实现了对内存的灵活管理.


![](C++.assets/image-20230310170353.png)



注意: 为了保持与之前的 16 位架构兼容, 32位 CPU 中的段寄存器仍然保持了16位宽度. 在实模式下, 可以使用 CS:IP 和 DS:BX 等寄存器对 32 位内存空间进行访问. 在保护模式下, 可以使用段选择子和段描述符来描述不同的内存逻辑段, 并通过段寄存器来访问它们.

每个段选择子都有一个对应的段描述符, 其中包含了内存段的基地址, 大小, 特权级别等信息.  通过设置特权级别, 可以限制某些代码或数据只能被特定的特权级别的程序访问, 从而提高了系统的安全性.

![](C++.assets/image-20230310170515.png)



虽然 32位 CPU 在保护模式中为分段访问内存机制引入了段描述符, 加强了段内存的安全性, 但是因为每个程序在运行时, 都会在完整的物理内存中, 获得一个专属且独立的进程内存区域(区域中含多个内存逻辑段), 因此当程序运行较多或程序段碎片过多时, 或是某段内存数据因为不常使用被换出, 想换入却找不到合适的大小时, 就暴露出了仅使用分段模式的不足. 
- 当一个进程需要分配内存时, 分段式内存管理需要保证一个进程的每个段都需要被分配到连续的物理内存地址空间中, 这可能会导致产生内部内存碎片. 例如, 如果一个进程的数据段需要分配10MB 的内存, 但是却只能找到一个 11MB 的连续空闲地址空间可用, 那么这个进程将浪费 1MB 的内存.
- 由于分段式内存管理需要将进程的物理内存划分为不同的逻辑段, 因此难以适应动态变化的内存需求. 例如, 如果一个进程的某个逻辑段需要更多的内存, 而另一个逻辑段则需要更少的内存, 这就需要重新分配和重定位段的基地址, 可能导致内存碎片和性能下降.
- 由于分段式内存管理将进程的物理内存划分为不同的逻辑段, 因此难以实现多个进程之间的共享内存. 共享内存是一种非常有用的机制, 它可以使得多个进程能够访问相同的内存块, 从而实现数据共享和通信. 在分段式内存管理中, 要实现共享内存, 需要使用复杂的机制, 如共享段和共享内存区域等, 而这些机制往往需要额外的系统开销和复杂性.


为了解决这种分段模式下, 进程中的内存地址等同于物理地址所带来的问题, 我们必须要将进程中的内存地址和物理内存地址解绑, 使得解绑以后得到的进程内存地址连续, 但物理地址可以不连续. 并且不同的进程内存地址都唯一对应一个物理地址.


为了实现这一要求, 操作系统将真实物理内存空间按照 “页” 为单位划分了很多页面, 这个页的大小默认是4KB（当然可以改的）, 并为每个进程构造了一个连续的虚拟内存空间, 其中所包含的虚拟内存页均与真实的物理内存页唯一对应. 其中连续的虚拟内存页对应的真实物理内存页可以不连续, 因为 CPU 在运行时可以很简单的自动将虚拟地址翻译成真实的物理地址.  这样, 各进程都会拥有连续的虚拟地址空间, 并且程序中使用的地址都是虚拟地址了.

![](C++.assets/image-20230310173941.png)

而虚拟地址与物理地址的对应关系是通过页表来完成的, 页表是用来记录虚拟内存页面和物理内存页面之间的映射关系的, 每一个页表项记录一个页面的映射关系. 

如果我们把一页的大小定义为 4KB,那么4G 就有 1M 个页, 在32位的保护模式下, 地址都是 32 位二进制表示的, 用 20 位二进制定位页表, 剩余的 12 位表示 4K 里面的偏移.
分页机制打开前, 要将页表地址加载到控制寄存器 CR3 中, 这个过程是打开页表之前, 所以存储的是物理实际地址, 每个页表项对应一个物理页, 通过页表项就可以访问到实际的物理地址. 由于这个过程是固定的, CPU 中集成了这个硬件模块, 即 MMU 中的页部件.

但进程的地址空间很大, 每个进程 1M 个页表，每个 4字节，进程多了占用的内存还是很多的, 这样算下来需要的页表项的数量也会非常多. 而实际上进程地址空间中很多页面都没有真正使用, 也就没有映射关系, 这样是一种浪费.

为了解决这个问题, CPU 引入了多级页表的机制, 在 32 位下一般是 2 级页表, 二级模式只为进程实际使用的那些虚拟内存区分配页表, 既提升了效率, 也减少了内存的使用量, 像下面这样:


![](C++.assets/image-20230310173428.png)


将虚拟地址划分了三段: 页目录索引, 页表索引, 页内偏移. 线程切换时, 如果同时发生了进程切换, CPU 中的 CR3寄存器将会加载当前进程的页目录地址.

在寻址的时候, 通过 CR3, 一级一级按表索页, 最终找到对应的物理内存页面, 再结合页面内的偏移值, 实现最终的内存寻址.

现代操作系统内存管理方式通常是分段+分页相结合的内存管理方式, 并且操作系统通过巧妙的设置, 屏蔽了段的存在.


对于程序申请的内存来说，一般都是虚拟内存，操作系统只需要负责给其分配足够的虚拟空间就好了，让程序以为自己具有这些内存空间，而实际意义上的物理内存，是等到非用不可的时候，才会使用的。

并且, 当程序访问某个内存区域时, 操作系统会将对应的页面加载到物理内存中, 程序就可以访问这些内存区域了. 如果物理内存不足, 操作系统会将一些页面写入硬盘中,  以腾出空间给需要使用的页面. 这个过程对程序员是透明的, 程序员只需要使用合适的方式分配和释放堆内存, 其它内存区域都是由编译器和操作系统自动管理的.

### 操作系统与 C 内存管理机制

操作系统通过上述的内存分页机制以及其他高速缓存方法, 将物理内存映射为连续的虚拟内存, 并提供了一些与虚拟内存相关的 API 接口方便对虚拟内存进行管理. 
每一个程序被启动时, 操作系统均会为其分配一块虚拟内存空间, 这个虚拟内存空间通常被称为进程空间, 并且每个进程的进程空间都是独立的, 与其他进程的虚拟内存空间相互隔离. 也就是说, 他们所使用的物理内存区域不会产生重叠. 



![](C++.assets/image-20230311155613.png)

通常来说, 程序进程的虚拟地址空间会被分为用户空间和内核空间两个部分，用户空间中包括了代码段、数据段、BSS 段、堆、栈、共享库区等部分。而内核空间则包含操作系统内核的代码和数据，例如中断处理程序、系统调用接口等。例如, 对于 32 位的 Linux 操作系统, 其程序进程的虚拟地址空间大小通常为2^32=4GB. 其中 0-3G 是用户空间, 3G-4G 是内核空间. 


![](C++.assets/image-20230311154542.png)


内核空间在页表中拥有较高特权级, 因此当用户程序试图访问这些内存页时会导致一个页错误(page fault). 在 Linux 中, 内核空间是持续存在的, 并被所有进程共享, 也就是说这一内存区域都映射到相同的物理内存区域. 内核代码和数据总是可寻址, 随时准备处理中断和系统调用. 与此相反, 用户空间的映射随进程切换的发生而不断变化. 

每个程序的用户空间中, 都会被再次分为多个逻辑段, 如栈区(Stack), 堆区, 数据区与代码区.

- **栈(stack)**: 程序进程的用户空间中最顶部的段是栈 (stack), 用来存储局部变量, 临时变量, 返回值, 函数参数, 函数的返回指针等. 当这些变量超出了当前作用域时将会自动弹出. 该栈的最大存储是有大小的, 该值固定, 超过该大小将会造成栈溢出. 操作方式类似于数据结构中的栈. 例如, 调用一个方法或函数时, 会将一个新的栈帧（stack frame）压入到栈中, 这个栈帧会在函数返回时被清理掉.
- **内存映射段:** 内核将将硬盘文件的内容直接映射到内存, 一种方便高效的文件 I/O 方式, 通常被用来加载动态库. 任何应用程序都可以通过 Linux  的 `mmap()` 系统调用或者 Windows 的 `CreateFileMapping()/MapViewOfFile()` 请求这种映射.
- **堆(heap)** : 指的是一个比较大的内存空间, 主要用于程序运行时的动态内存分配. 在程序开发中一般是开发人员进行分配与释放, 若在程序结束时都未释放, 系统将会自动进行回收. 分配方式类似于链表. 
...
bss 段、数据段和代码段是可执行程序编译时的分段. bss 段、数据段和代码段是可执行程序编译时的分段

在 Windows 中，虚拟内存分配是通过 `VirtualAlloc` , `VirtualFree` 等函数实现的, 可以动态地分配或释放虚拟内存. 而在 Linux 中，虚拟内存分配是通过 `brk` 和 `mmap` 等系统调用实现的, 也可以动态地分配或释放虚拟内存. 而 C/C++ 的内存管理机制( `malloc` , `free` )就构建在这些操作系统的虚拟内存 API 之上. 

# 八股文

## const 关键字

const 的作⽤： 被它修饰的值不能改变，是只读变量。必须在定义的时候就给它赋初值。

### 底层 const （常量指针）

是指定义了⼀个指针，这个指针存储的地址所指向的对象（内存区域）为仅可读，因此不能通过此常量指针来改变这个对象（此区域存储）的值。

即：常量指针强调的是指针对其所指对象的不可改变性。
...
指针常量是指定义了⼀个指针，这个指针中存储的地址无法被改变，也因此指针常量必须要在定义时初始化。  
即：指针常量强调的是指针的不可改变性。
...
1. 通过变量的指针进行修改
	...
	int * p = (int *)&age;
	*p = 40;
	// 只能修改局部变量, 因为 age 的地址指向是堆栈中的区域, 可修改, 并且因为常量折叠原因(编译器会预先缓存常量到符号表,即变量地址还是源地址, 变量值直接用符号表的), 带有 const 属性的变量 age 计算时会继续使用符号表的值.
	// 不能修改全局变量, 因为其存放的内存区域也是只读的
	```
2. 在有些编译器中可以提前通过 `volatile` 关键字, 使得 `const` 失效
	```c++
	const volatile int age = 39;
	age = 40;
	```
3. 通过强制类型转换 `const_cast` 消除 `const` 属性
```c++
const int age = 39;
int * p = &age; // 使用类型转换消除 const 属性
...
```c++
// #define 是一个宏定义命令，它用来将一个标识符定义为一个字符串或是定义一个带参数的宏定义
// #define 标识符 字符串
#define PI 3.1415926
// #define 宏名(参数表) (字符串)
#define S(a, b) a*b
...
1. 在编译过程中起作用，先将内联函数编译完成，生成函数体，直接插入到被调用的地方，减少了普通函数调用时额外的压栈，跳转和返回的操作
2. 内联函数是⼀种特殊的函数，会进行类型检查
3. inline 只是对编译器的⼀种建议请求，能否真正内联还需要看编译器的意思
4. 有多种编译限制
   - 不能存在任何形式的循环语句
   - 不能存在过多的条件判断语句
   - 函数体不能过于庞大
   - 内联函数声明必须在调用语句之前
5. 直接定义在类中的成员函数默认都是 inline 的。对于仅声明，而外部定义的成员函数，可以通过在类外要加上 inline 使其变为内联函数
6. 必须与函数定义体放在一起才能使函数成为内联，仅将 inline 放在函数声明前面不起任何作用。
...
重写（覆盖）了⼀个⽅法 以实现不同的功能，⼀般是⽤于⼦类在继承⽗类时，重写⽗类⽅法。  
...
重载，这些⽅法的名称相同⽽参数形式不同，⼀个⽅法有不同的版本，存在于⼀个类中。
...
指针存放某个对象的地址，其本身是变量，可以指向空值；  
引⽤是变量的别名，从⼀而终，不可变，必须初始化，不存在指向空值的引⽤；

## new 和 malloc

- `malloc`/`free` 是标准的 C 语言函数(CRT), C++ 语⾔的标准库函数, `new`/`delete` 是 c++ 中定义的运算符, 是对 `malloc`/`free` 的进一步封装.
- `new` 操作符从⾃由存储区上为对象动态分配内存空间, ⽽ `malloc` 函数从堆上动态分配内存.
...
- opeartor `new` /operator `delete` 可以被重载, ⽽ `malloc` / `free` 并不允许重载.

## constexpr 和 const

两者都被用来为变量或函数添加只读属性, 但 `constexpr` 只能定义编译期常量, 即在程序编译期就可以被直接求出的值, ⽽ `const` 可以定义编译期常量和运⾏期常量, 即在程序运行时才能被求出的值.

- 如果使用 `constexpr` 声明定义了⼀个变量, 那么在编译过程中, 编译器就会验证此变量的值是否是⼀个常量表达式.
    ```c++
    constexpr int n = 20; 
    constexpr int m = n + 1;
    static constexpr int MOD = 1000000007;
     ```
- 如果使用 `constexpr` 声明定义了⼀个指针, 则 `constexpr` 仅对指针有效, 和所指对象⽆关.
	```c++
	constexpr int *p = nullptr;   // 常 量 指 针 顶 层 
	const const int *q = nullptr; // 指向常量的指针， 底层
	const int *const q = nullptr; // 顶层 const
	```
- 如果使用 `constexpr` 声明定义了⼀个函数, 那么函数的**返回类型**和**所有形参类型**都必须是 [字面值类型](C++.md#^1cc9cf), 函数体有且只有⼀条 `return` 语句, 并且通常会被隐式转换成内联函数, 从而避免函数调用的开销, 进一步提高程序的执行效率.
	```c++
	constexpr int new() {return 42;}
	```
- 如果使用 `constexpr` 声明定义了⼀个类构造函数, 则此构造函数必须有一个空的函数体, 也就是说所有成员变量的初始化都放到初始化列表中. 并且此类对象的成员函数必须使用          `constexpr` 修饰.

## const 和 static 的区别

## volatile 

⽤该关键字声明的变量表⽰该变量随时可能发⽣变化, 因此与该变量有关的运算, 不要进⾏编译优化, 而是应该直接从内存中重新装载内容.

```c++
for(volatile int i=0; i<100000; i++); // 避免优化掉空循环
```
...
实现多个对象之间的数据共享 + 隐藏, 并且使⽤静态成员还不会破坏隐藏原则, 默认初始化为 0.
...
## `a++` , `int a = b` 等基础操作在 C++中是否是线程安全的？

不是

从 C/C++ 语法的级别来看, 这些非常基本的语句, 但从编译器得到的汇编指令来看, 它们其实不是原⼦的. 由于操作系统线程调度的不确定性, 即使只有三条汇编指令, 也可能在多线程同时对某一对象操作时, 发生不确定情况.

以 `a++` 举例, 其⼀般对应三条汇编指令, ⾸先将变量 `a` 对应的内存值搬运到某个寄存器（如 `eax` ）中, 然后将

该寄存器中的值⾃增1, 再将该寄存器中的值搬运回 `a` 代表的内存中. 
...
- 类的内部, ⽆论成员被声明为 public, protected 还是 private, 都是可以互相访问的, 没有访问权限的限制.
- 类的外部, 只能通过对象访问类内成员, 并且只能访问 public 属性的成员, 不能访问 private, protected 属性的成员.
- 在面向对象的继承关系中, private 成员不能被 "子类" 访问, 而基类中的 public 和 protected 类型成员均能被 "子类" 访问.
- public 类型的继承关系中, 父类的 public 成员可以被 "子类" 对象访问, 而 private, protected 成员不可被 "子类" 对象访问.
- private 和 protected 类型的继承关系中, 父类的所有成员都不可以被子类访问.

## 面向对象

### 继承

让某种类型对象获得另⼀个类型对象的属性和⽅法, 在具体实现过程中, 子类可以选择完全继承或者部分继承, 即可以自定义与父类同名称的属性和方法.

### 封装

把变量和函数封装成抽象的类, 并且类可以控制自身成员的访问权限, 如 public, protected, private.

### 多态

子类继承自父类对象, 但是却表现出与父类对象不同的能⼒, 即子类可以重载或覆盖父类的成员函数, 同时因为子类继承了父类的属性和方法, 可以使用父类的指针来直接引用子类对象, 并调用其重写的父类方法, 而无需直到其具体类型. 

实现多态有两种⽅式: 
override(重写): 是指⼦类重新定义⽗类的虚函数的做法, 虚函数实现运⾏时多态
overload(重载): 是指允许存在多个输入参数不同的同名函数存在, 重载实现编译时多态
...
如何定制删除动作
在单链表中使用智能指针，链表稍微长一些就会栈溢出（思考题：为什么？）
在双向链表中用shared_ptr，循环引用
在双向链表中用一个 weak_ptr 打破循环引用，会出现1的问题，且破坏了节点对称性
在双向链表中用两个weak_ptr

在树形结构中使用shared_ptr，如果子节点引用父节点使用weak_ptr，首先这个树基本没办法移除非叶子节点，更别提复杂的旋转操作，其次释放树的顺序只能先释放父节点再释放子节点，而实际上的释放都是先释放挂件再释放主体。思考题：在这个例子中把share和weak交换会怎样？

在有环的图结构使用智能指针，能够触发上述所有问题，要不要写个算法检测循环引用？
函数参数返回值使用智能指针，可能有 abi 问题，另外智能指针的代价很大，远远不是整数加一减一那么简单

c++03。主要是 c，虚函数，继承，stl 的容器。不需要懂实现。
c++11。RAII、构造函数、auto、右值引用、lambda、template。
c++17。代数类型、pmr 分配器（看需求），掌握 oop 常用的设计模式。
c++20。concept、模版元编程、预编译处理器元编程。至此学完 c++同步编程。
c++23。executors、coroutine。

## 虚函数

> [!cite]
> [保护模式篇——段描述符与段选择子](https://www.cnblogs.com/wingsummer/p/15312627.html)
> [现代操作系统内存管理到底是分段还是分页](https://zhuanlan.zhihu.com/p/409754117)
> [FS/GS寄存器的用途](https://zhuanlan.zhihu.com/p/435518616)
> [linux进程地址空间划分](https://www.cnblogs.com/yizhanwillsucceed/p/13578076.html)
> [深度阅读：深入 C++ 内存管理（万字长文）](https://zhuanlan.zhihu.com/p/344377490)
> [游戏引擎开发新感觉！(6) c++17内存管理 ](https://zhuanlan.zhihu.com/p/96089089)
> [浅谈C++内存管理](https://zhuanlan.zhihu.com/p/51855842)
> [linux内存管理之malloc的实现 ](https://zhuanlan.zhihu.com/p/448535361)
> [操作系统之内存管理](https://zhuanlan.zhihu.com/p/359165294)
> [malloc申请一块内存的背后原理是什么？](https://www.zhihu.com/question/33979489)

# 项目学习

## 并发编程之线程池(Thread Pool)

- 一组预先创建的线程，这些线程可以在需要时被重复利用。
- 线程池维护一个线程队列，并根据需要将任务分配给可用线程来执行。
- 线程池可以控制并发执行的线程数量，避免线程过多导致资源消耗和性能问题。
- 线程池通常用于管理并发任务的执行，提供了一种可控的线程管理方式。

线程池, 建议用于异步和并行编程的 C++ 框架. C++ 一直缺乏可用的并发编程的标准库, 并且自 C++11 以来新引入的标准, 以及 boost, folly 等第三方库对标准库的改进，都有或多或少的问题和一定的局限性.

https://github.com/facebookexperimental/libunifex/blob/main/include/unifex/static_thread_pool.hpp
https://github.com/facebook/folly/blob/main/folly/executors/ThreadPoolExecutor.h
https://github.com/chengxumiaodaren/wzq_utils/blob/master/thread/include/thread/thread_pool.h

## 并发编程之 Executor

- 一个接口，定义了用于执行异步任务的标准接口。
- Executor 框架提供了一种将任务提交给执行者的方式，并且隐藏了任务执行的细节。
- Executor 框架通过使用线程池来执行提交的任务，提供了一种简化线程管理和任务调度的方式。
- Executor 框架支持各种不同类型的任务执行方式，包括同步执行、异步执行、延迟执行等。



[folly/folly/executors at main · facebook/folly · GitHub](https://github.com/facebook/folly/tree/main/folly/executors)
[GitHub - facebookexperimental/libunifex: Unified Executors](https://github.com/facebookexperimental/libunifex/)
[GitHub - chriskohlhoff/executors: C++ library for executors](https://github.com/chriskohlhoff/executors/)
https://github.com/NVIDIA/stdexec 
[开发友好的低配版 executors 标准库 ](https://github.com/Caturra000/legacy-executors)



> [!cite]
> [浅谈The C++ Executors](https://zhuanlan.zhihu.com/p/395250667)
> [A Unified Executors Proposal for C++ | P0443R14](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2020/p0443r14.html)
> [C++ Executors: the Good, the Bad, and Some Examples](https://accu.org/journals/overload/29/164/teodorescu/)
> [GitHub - executors/executors: A proposal for a executor programming model for ISO C++](https://github.com/executors/executors)

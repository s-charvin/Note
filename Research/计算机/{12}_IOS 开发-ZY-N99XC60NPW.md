...
- `addConstraint(_:)`  定义视图及其子视图的自动布局行为
- `autoresizingMask` 自动布局
- `contentMode` 为视图内部内容设置布局行为
...
- `traitOverrides: UITraitOverrides { get set }`  通过 `UITraitOverrides`  为特定的视图配置外观和布局属性, 覆盖原有的界面特征
...
UIControl 定义特定于按钮、开关和其他为用户交互而设计的视图的附加行为。
UIButton
UILabel
UITextView
UITextField

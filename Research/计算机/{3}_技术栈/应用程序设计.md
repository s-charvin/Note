...
- 更加方便地通过模拟数据进行测试
- 使用方式更接近于语言(如 Swift)的标准 API
- 编译阶段就能防止隐藏的崩溃问题。如果应用程序构建，则表示所有依赖项都已正确配置.
- 避免由于注入依赖项而导致的复杂初始化
- 在不暴露包内的私有/内部类型的情况下，应该能够定义标准依赖关系。

**依赖注入的类型：**

1. 通过初始化器进行依赖注入（初始化器注入）

通过构造器注入，确保了类的实例在创建时就已经拥有所有必要的依赖项, 具有**明确的依赖关系**。
...
class LogInView {
    ...
    init(authService: AuthenticationService, loginSwitcher: LogInSwitcher) {
    ...
    func login(username: String, password: String) {
        authService.authenticate(username: username, password: password) { [weak self] success in
            if success {
                self?.loginSwitcher.switchToAuthenticated()
            } else {
                self?.loginSwitcher.switchToUnauthenticated()
            }
        }
    }
}
...
let switcher = AppLogInSwitcher()
let loginView = LogInView(authService: authService, loginSwitcher: switcher)

loginView.login(username: "user", password: "password")

```

2. dependency injection using properties (property injection)  
    使用属性的依赖注入（属性注入）

3. dependency injection in methods (method injection)  
    方法中的依赖注入（方法注入）

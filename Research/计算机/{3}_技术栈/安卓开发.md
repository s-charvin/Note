...
│   │   ├── business_lib // 业务库
│   │   ├── business_app // 业务 app
│   │   ├── business_plugin // 业务插件
```

Shadow 包含编译时和运行时两部分, 同时又区分了宿主和插件两种不同的角色.

编译时宿主会通过 `core/gradle-plugin` 中 的 `ShadowPlugin` 插件自动生成插件化框架所需的配置文件, 插件信息, 辅助类文件等.

编码时:

- 依赖 `com.tencent.shadow.core:common` 和 `com.tencent.shadow.dynamic:dynamic-host`
- `AndroidManifest.xml`
    - 自定义和注册 `PluginProcessService` 类型的插件进程服务, 用于管理插件加载过程, 每个插件进程都需要注册一个.
    - 自定义和注册 `PluginLoadActivity`, 用于承接插件加载逻辑.
    - 自定义和注册 `XxxxxProxyActivity`, 为插件内业务 Activity 进行系统事件转发和处理, 可以有多个, 每个是不同的启动模式.
...
- `Application`
    - `onCreate`:
        ...
        - 如果是宿主进程, 则初始化插件框架, 管理插件加载器, 以及其他 SDK 相关的初始化.
            - 通过 Provider 为插件提供宿主的可共享资源.
        - 通过 `PluginManagerProvider` 创建和获取插件管理器(PluginManager)实例
            - 自定义 `PluginManagerUpdater` 管理插件 apk 文件的更新和安装.
            - 自定义 `DynamicPluginManager` 管理插件的加载(`enter`)和卸载.
- `MainActivity`
    - `onCreate`:
        - 通过 `PluginManager` 获取插件的 `PluginInfo` 信息, 包括插件的包名, 版本号, 签名信息等.

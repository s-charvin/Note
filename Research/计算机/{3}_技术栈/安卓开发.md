...
> [https://developer.android.com/courses/unit/kotlin-basics-unit-1](https://developer.android.com/courses/unit/kotlin-basics-unit-1)
> [https://www.geeksforgeeks.org/kotlin-android-tutorial/](https://www.geeksforgeeks.org/kotlin-android-tutorial/)
...
> [!cite]
> [Android 测试模块](https://developer.android.com/topic/modularization/patterns#test-modules)
> [Android 测试](https://developer.android.com/training/testing?hl=zh-cn)
...
![布局图片](https://developer.android.com/static/codelabs/basic-android-kotlin-compose-add-images/img/df69881d07b064d0.gif?hl=zh-cn)
...
![事件分发](https://img2018.cnblogs.com/blog/653161/201903/653161-20190321113420853-1974821013.png)
![事件分发流程](https://juejin.cn/post/7102248400019521544)
...
- `ViewGroup.dispatchTouchEvent`
  ...
  - 首先进行事件拦截的判断和处理. 事件拦截, 会导致事件不再向子视图传递.
    - 拦截条件1: 1. 是 `ACTION_DOWN` 事件, 或者已有子视图在处理触摸事件; 2. `FLAG_DISALLOW_INTERCEPT` 标志未被设置为 true; 拦截成功后会调用 `onInterceptTouchEvent` 方法. 后续不会再进行向子视图的触摸事件分发.
    - 拦截条件2: 不是 `ACTION_DOWN` 事件, 且没有任何子视图在处理当前触摸事件. 也视为拦截成功. 后续不会再进行向子视图的触摸事件分发.
...
> [Android主流网络请求框架 OkHttp 全方位详析 - 掘金](https://juejin.cn/post/7345756317557817354?searchId=20240806152219108948FAF7C05048547D)
...
![Request](安卓开发.assets/file-20240806154800379.png)
...
![Okhttp](安卓开发.assets/file-20240806152547281.png)
...
假设要从GitHub拉取OpenCV：[https://github.com/opencv/opencv进行编译](https://github.com/opencv/opencv%E8%BF%9B%E8%A1%8C%E7%BC%96%E8%AF%91), 可以按照以下的方法.
...
在Linux系统下, 可以写一个 shell 脚本, 自动循环进行 `git fetch`.
...
> [BRouter](https://github.com/bilibili/BRouter)
> [Custom Router](https://github.com/gdutxiaoxu/Router)
> [Custom Router](https://github.com/waws80/Router)
> [Custom Router](https://github.com/chenenyu/Router)
> [Android路由框架用法大全](https://juejin.cn/post/6984993972804550692)
> [Android 组件化 —— 路由设计最佳实践](https://cloud.tencent.com/developer/article/1356515)
> [Android 客户端路由框架的整理和思考](https://www.jianshu.com/p/15d8cc6cf19b)
> [GRouter](https://github.com/taoweiji/grouter-android/)
...
| **IDE插件支持**      | 提供 Android Studio 插件，便于路径跳转                           | 可通过 ARouter Helper 插件快速跳转到目标类，提高开发效率             |
...
- `RouteMeta` 数据类, 用于封装路由信息, 包括路径、组、名称、类型、优先级、降级策略、拦截器、参数等信息.
    ...
    - `priority: int`: 路由优先级.
    - `extra: int`: 路由附加信息.
    - `paramsType: Map<String, Integer>`: 路由参数类型.
    - `name: String`: 路由名称.
    - `injectConfig: Map<String, Autowired>`: 路由注入配置.

##### arouter-compiler

实现了注解处理器, 用于在编译时扫描组件内所有使用了预设注解的类, 属性, 或方法, 然后根据注解中的信息生成一个路由表, 包含路径到目标类的映射关系和参数信息.

项目中要处理注解自动生成路由表代码, 则必须依赖 `arouter-compiler` 模块, 并在 `build.gradle` 文件中配置 `kapt` 插件.
...
支持生成路由文档, 通过 `AROUTER_GENERATE_DOC` 参数开启文档生成. 最终生成的文档是一个 JSON 格式的文件, 包含了所有模块的路由信息. 文档生成的路径是在 `build/generated/source/apt` 目录下. 文件名为 `arouter-map-of-模块名.json`.
...
`@Route` 注解用于标注路由信息(如路径、组、名称、优先级、附加标志位). 
- 路由只支持类注解, 包括 Activity, Fragment, Service, IProvider 类型.
- 路由信息必须包含路径信息, 同时以 `/` 开头.
- 路由组信息用于分组加载路由信息, 不提供则要求路径至少包含两级, 采用第一级作为组名.

同时 Activity 支持 `@Autowired` 注解自动注入目的对象内部的属性值.

- `groupMap: Map<String, List<RouteMeta>>` 用于分组存储不同模块和其内包含的路由信息.
- `rootMap: Map<String, String>` 用于存储根路由信息, 用于生成类文件时的顺序.
- `iProvider: TypeElement` 用于存储 `IProvider` 接口的元素信息.
...
for (groupName, routeMetas in groupMap):
    rootMap.put(group, group + "Root"); // 生成根路由信息

// 构造路由信息
ParameterizedTypeName inputMapTypeOfRoot: Map<String, Class<? extends IRouteGroup>>
ParameterizedTypeName inputMapTypeOfGroup: Map<String, RouteMeta>
MethodSpec.Builder loadIntoMethodBuilder: PUBLIC Override loadInto(Map<String, Class<? extends IRouteGroup>> routes)
MethodSpec.Builder loadIntoMethodBuilder: PUBLIC Override loadInto(Map<String, RouteMeta> atlas)
MethodSpec.Builder loadIntoMethodBuilder: PUBLIC Override loadInto(Map<String, RouteMeta> providers)
```

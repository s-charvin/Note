...
- `ViewGroup.dispatchTouchEvent`
    ...
    - 当事件是 `ACTION_DOWN`(初始按下) 或 `ACTION_POINTER_DOWN`(多点触控新增的手指按下) 或 `ACTION_HOVER_MOVE`(按下后移动) 时, 先逆序查找所有可以接收事件的子视图 (视图是否可见、能否接收事件) 并调用子视图的 `dispatchTouchEvent` 方法传递事件, 直到有子视图消费事件或者事件被取消. 而如果没有子视图消费事件, 则尝试将要处理的事件分发给第一次按下时保存的所有可以处理事件的子视图链表中的.
    - 
...
#### `onRequestDisallowInterceptTouchEvent` 方法
...
val animatorSet = AnimatorSet().apply {
    ...
    playTogether(objectAnimator2, objectAnimator3)
...
val animator = AnimatorInflater.loadAnimator(context, R.animator.animator_file)
...
估值器（TypeEvaluator）决定了动画属性值在起始值和结束值之间随进度 (fraction) 而如何变化. 动画期间, 估值器会根据当前时间对应的动画进度来计算出当前属性值, 并将其传递给动画监听器, 从而实现动画效果.

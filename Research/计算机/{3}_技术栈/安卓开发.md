...
- `init` 方法: 线程安全, 初始化 ARouter 框架, 加载路由表(实际加载由 `LogisticsCenter` 执行)
    - `getInstance` 方法: 双检锁获取 ARouter 单例对象实例.
        ...
        - `navigation(context, postcard, requestCode, callback) -> Object` 方法: 不需要 path, 直接通过自行构造的 Postcard 对象进行 Activity 页面跳转.
...
- `ARouter` 的 `build` 路由请求构造过程可选手动设置的参数:
...
- `ARouter` 的 `navigation` 过程自动设置的参数:
    - `type: RouteType`: 路由类型.
    - `rawType: Element`: 路由目标类原始信息
    - `destination: Class<?>`: 路由目标类具体
    - `priority: int`: 路由优先级.
